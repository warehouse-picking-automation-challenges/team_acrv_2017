...
rs_context_base::rs_context_base(const std::string &serial_number)
{
    context = rsimpl::uvc::create_context();
    ...
    for(auto device : query_devices(context))
    {
        ...
        try {
            ...
            if (rs_dev && is_compatible(rs_dev) && rs_dev->get_serial() == serial_number)
            {
                devices.push_back(rs_dev);
                break;
            }
        ...
        } catch (...)  // TODO FIX BAD PRACTICE CATCH ALL
        {
            continue;
        }
    }
...
rs_context* rs_context_base::acquire_instance(const std::string &serial_number)
{
    std::lock_guard<std::mutex> lock(instance_lock);
    if (ref_count++ == 0)
    {
        instance = new rs_context_base(serial_number);
    }
    return instance;
}
