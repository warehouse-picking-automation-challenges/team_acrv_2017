...
rs_context * rs_create_context_serial(int api_version, rs_error ** error, const char *serial_number) try
{
    rs_error * local_error = nullptr;
    auto runtime_api_version = rs_get_api_version(&local_error);
    if (local_error) throw std::runtime_error(rs_get_error_message(local_error));
    ...
    return rs_context_base::acquire_instance(std::string(serial_number));
}
HANDLE_EXCEPTIONS_AND_RETURN(nullptr, api_version)
